...
n_points = 101          # RINN输入频率分辨率，与data目录中的频率点数量一致
f_start = 10.50e9       # 起始频率
f_stop = 11.50e9        # 终止频率
...
def generate_ideal_response(cfg, result_dir='result_RINN_input'):
    """
    生成理想的滤波器响应
    ...
    # 1. 生成频率轴，与data目录中的频率点一致
    # 从10.5 GHz到11.5 GHz，步长为0.01 GHz，共101个点
    freq = np.arange(10.5, 11.51, 0.01) * 1e9  # 转换为Hz
    nyq = 0.5 * fs
    ...
    # 7. 保存结果
    transformed_data = save_results(freq, S11_dB, S11_real, S11_imag, passband_mask, 
                target_dB, best_rp, best_S, best_diff, results, result_dir)
...
def save_results(freq, S11_dB, S11_real, S11_imag, passband_mask, 
                 target_dB, best_rp, best_S, best_diff, results, result_dir):
    """
    保存生成的结果
    ...
    # ===================== 计算变换后的数据 =====================
    # 1. 相反数变换
    S11_real_neg = -S11_real
    S11_imag_neg = -S11_imag
    
    # 2. 中心频率对称变换
    center_idx = len(freq) // 2
    S11_real_sym = np.flip(S11_real)
    S11_imag_sym = np.flip(S11_imag)
    ...
    np.savez(data_path,
             ...
             S11_real=S11_real,
             S11_imag=S11_imag,
             S11_real_neg=S11_real_neg,
             S11_imag_neg=S11_imag_neg,
             S11_real_sym=S11_real_sym,
             S11_imag_sym=S11_imag_sym,
    ...
    print(f"📊 图像保存至: {save_path_real_imag}")
    ...
    return {
        'original': {'real': S11_real, 'imag': S11_imag},
        'negative': {'real': S11_real_neg, 'imag': S11_imag_neg},
        'symmetric': {'real': S11_real_sym, 'imag': S11_imag_sym}
    }
